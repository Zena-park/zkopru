...
const WITHDRAWAL_DELAY_RINKEBY = Math.floor(
  WITHDRAWAL_DELAY_MAINNET / (14 * 24 * 6),
) // 10 min

// 1209600 sec (= 2 weeks)
const ROUND_DURATION_MAINNET = 1209600
const ROUND_DURATION_RINKEBY = Math.floor(
  ROUND_DURATION_MAINNET / (14 * 24 * 6),
) // 10 min
...
module.exports = async function(deployer, network) {
  if (process.env.DEPLOY_PLASMA_EVM) {
    ...
    let wtonAddress
    let registryAddress
    let depositManagerAddress
    let factoryAddress
    let daoVaultAddress
    let seigManagerAddress
    let powertonAddress
    let ton
    ...
    if (tonAddress === undefined) {
      ...
      await deployer.deploy(TON).then(_ton => {
        ton = _ton
    ...
    if (wtonAddress === undefined) {
      ...
      await deployer.deploy(WTON, tonAddress).then(_wton => {
        wton = _wton
    ...
    if (registryAddress === undefined) {
      ...
      await deployer.deploy(Layer2Registry).then(_registry => {
        registry = _registry
    ...
    if (factoryAddress === undefined) {
      ...
      await deployer.deploy(CoinageFactory).then(_factory => {
        factory = _factory
    ...
    if (daoVaultAddress === undefined) {
      ...
      await deployer
        .deploy(OldDAOVaultMock, wtonAddress, 1609416000)
        .then(_daoVault => {
          daoVault = _daoVault
